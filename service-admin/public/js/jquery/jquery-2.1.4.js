...
jQuery.extend = jQuery.fn.extend = function() {
	const options, name, src, copy, copyIsArray, clone,
		target = arguments[0] || {},
		i = 1,
...
jQuery.each({
	...
	jQuery.fn[ name ] = function( until, selector ) {
		const matched = jQuery.map( this, fn, until );
...
jQuery.Callbacks = function( options ) {
	...
	const // Last fire value (for non-forgettable lists)
		memory,
...
jQuery.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		...
		const handleObjIn, eventHandle, tmp,
			events, t, handleObj,
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		const i, cur, tmp, bubbleType, ontype, handle, special,
	...
	dispatch: function( event ) {
		...
		const i, j, ret, matched, handleObj,
			handlerQueue = [],
...
function cloneCopyEvent( src, dest ) {
	const i, l, type, pdataOld, pdataCur, udataOld, udataCur, events;
...
jQuery.extend({
	clone: function( elem, dataAndEvents, deepDataAndEvents ) {
		const i, l, srcElements, destElements,
	...
	buildFragment: function( elems, context, scripts, selection ) {
		const elem, tmp, tag, wrap, contains, j,
...
jQuery.fn.extend({
	...
	domManip: function( args, callback ) {
		...
		const fragment, first, scripts, hasScripts, node, doc,
			i = 0,
			l = this.length,
			set = this,
			iNoClone = l - 1,
...
(function() {
	const pixelPositionVal, boxSizingReliableVal,
...
function ajaxHandleResponses( s, jqXHR, responses ) {
	...
	const ct, type, finalDataType, firstDataType,
		contents = s.contents,
