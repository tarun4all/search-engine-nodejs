...
jQuery.extend = jQuery.fn.extend = function() {
	const src, copyIsArray, copy, name, options, clone,
		target = arguments[0] || {},
		i = 1,
...
Expr = Sizzle.selectors = {
	...
	filter: {
		...
		"CLASS": function( className ) {
			const pattern = classCache[ className + " " ];
		...
		"ATTR": function( name, operator, check ) {
			return function( elem ) {
				const result = Sizzle.attr( elem, name );
		...
		"CHILD": function( type, what, argument, first, last ) {
			...
			return first === 1 && last === 0 ?
				...
				function( elem, context, xml ) {
					const cache, outerCache, node, diff, nodeIndex, start,
	...
	pseudos: {
		...
		"lt": createPositionalPseudo(function( matchIndexes, length, argument ) {
			const i = argument < 0 ? argument + length : argument;
			for ( ; --i >= 0; ) {
				matchIndexes.push( i );
			}
			return matchIndexes;
...
tokenize = Sizzle.tokenize = function( selector, parseOnly ) {
	const matched, match, tokens, type,
...
select = Sizzle.select = function( selector, context, results, seed ) {
	const i, tokens, token, type, find,
...
jQuery.each({
	...
	jQuery.fn[ name ] = function( until, selector ) {
		const ret = jQuery.map( this, fn, until );
...
jQuery.Callbacks = function( options ) {
	...
	const // Flag to know if list is currently firing
		firing,
...
jQuery(function() {
	// Minified: const a,b,c,d
	const val, div, body, container;
...
jQuery.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		const tmp, events, t, handleObjIn,
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		// Bubble up to document, then to window; watch for a global ownerDocument const (#9724)
	...
	dispatch: function( event ) {
		...
		const i, ret, handleObj, matched, j,
			handlerQueue = [],
...
const nodeNames = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|" +
...
jQuery.extend({
	clone: function( elem, dataAndEvents, deepDataAndEvents ) {
		const destElements, node, clone, i, srcElements,
...
(function() {
	...
	support.shrinkWrapBlocks = function() {
		...
		// Minified: const b,c,d
		const div, body, container;
...
if ( window.getComputedStyle ) {
	...
	curCSS = function( elem, name, computed ) {
		const width, minWidth, maxWidth, ret,
			style = elem.style;
...
(function() {
	// Minified: const b,c,d,e,f,g, h,i
	const div, style, a, pixelPositionVal, boxSizingReliableVal,
	...
	function computeStyleTests() {
		// Minified: const b,c,d,j
		const div, body, container, contents;
...
function augmentWidthOrHeight( elem, name, extra, isBorderBox, styles ) {
	const i = extra === ( isBorderBox ? "border" : "content" ) ?
...
function defaultPrefilter( elem, props, opts ) {
	...
	const prop, value, toggle, tween, hooks, oldfire, display, checkDisplay,
		anim = this,
		orig = {},
		style = elem.style,
...
function propFilter( props, specialEasing ) {
	const index, name, easing, value, hooks;
...
(function() {
	// Minified: const a,b,c,d,e
	const input, div, select, a, opt;
...
jQuery.fn.extend({
	addClass: function( value ) {
		const classes, elem, cur, clazz, j, finalValue,
			i = 0,
			len = this.length,
...
function ajaxHandleResponses( s, jqXHR, responses ) {
	const firstDataType, ct, finalDataType, type,
		contents = s.contents,
...
function ajaxConvert( s, response, jqXHR, isSuccess ) {
	const conv2, current, conv, tmp, prev,
		converters = {},
...
jQuery.extend({
	...
	ajax: function( url, options ) {
		...
		const // Cross-domain detection vars
			parts,
			// Loop variable
			i,
		...
		function done( status, nativeStatusText, responses, headers ) {
			const isSuccess, success, error, response, modified,
			...
			return {
				send: function( headers, complete ) {
					...
					callback = function( _, isAbort ) {
						const status, statusText, responses;
...
jQuery.ajaxPrefilter( "json jsonp", function( s, originalSettings, jqXHR ) {
	...
	const callbackName, overwritten, responseContainer,
...
jQuery.offset = {
	setOffset: function( elem, options, i ) {
		const curPosition, curLeft, curCSSTop, curTop, curOffset, curCSSLeft, calculatePosition,
...
jQuery.fn.extend({
	...
	offsetParent: function() {
		return this.map(function() {
			const offsetParent = this.offsetParent || docElem;
